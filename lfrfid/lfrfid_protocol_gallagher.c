...
/* Gallagher scramble table (256 entries) — forward LUT for encoding/write */
static const uint8_t gallagher_scramble_table[256] = {
...
/*============================================================================*/
/* T5577 write support — ported from Flipper protocol_gallagher_write_data /
 * protocol_gallagher_encoder_start (lib/lfrfid, GPLv3).
 *
 * Gallagher: ASK/Manchester modulation, RF/32 bitrate, 3 data blocks. The
 * 96-bit encoded frame is preamble (0x7FEA) + 8 scrambled data bytes each
 * followed by an inverse-of-last-bit separator + a CRC-8 byte. The decoded
 * card (8 bytes) does NOT fit tag->uid[5], so it is sourced from this
 * protocol's full decoded static buffer g_gallagher_decoded, whose byte
 * layout matches Flipper's protocol->data verbatim:
 *   [0] = region(4) | issue_level(4), [1..3] = FC (24b BE), [4..7] = card (32b BE).
 */
/*============================================================================*/
static void gallagher_scramble(uint8_t *data, size_t length)
{
    for (size_t i = 0; i < length; i++)
        data[i] = gallagher_scramble_table[data[i]];
}

static void gallagher_encoder_start(const uint8_t *decoded, uint8_t *encoded)
{
    /* Preamble 0x7FEA */
    bit_lib_set_bits(encoded, 0, 0x7F, 8);
    bit_lib_set_bits(encoded, 8, 0xEA, 8);

    uint8_t  rc = bit_lib_get_bits(decoded, 0, 4);
    uint8_t  il = bit_lib_get_bits(decoded, 4, 4);
    uint32_t fc = bit_lib_get_bits_32(decoded, 8, 24);
    uint32_t cn = bit_lib_get_bits_32(decoded, 32, 32);
    ...
    /* Gallagher scramble */
    gallagher_scramble(payload, 8);

    for (int i = 0; i < 8; i++) {
        /* data byte */
        bit_lib_set_bits(encoded, 16 + (i * 9), payload[i], 8);
        /* every byte is followed by a bit which is the inverse of the last bit */
        bit_lib_set_bit(encoded, 16 + (i * 9) + 8, !(payload[i] & 0x1));
    }

    /* checksum: CRC-8 poly=0x07, init=0x2C */
    uint8_t crc = (uint8_t)bit_lib_crc8(payload, 8, 0x07, 0x2C, false, false, 0x00);
    bit_lib_set_bits(encoded, 16 + (9 * 8), crc, 8);
}

void gallagher_write_begin(void *protocol, void *data)
{
    (void)protocol;  /* data sourced from g_gallagher_decoded, not tag->uid */
    LFRFIDProgram *write = (LFRFIDProgram*)data;
    uint8_t        encoded[GALLAGHER_ENCODED_SIZE];

    memset(encoded, 0, sizeof(encoded));
    gallagher_encoder_start(g_gallagher_decoded, encoded);

    if (write && write->type == LFRFIDProgramTypeT5577) {
        write->t5577.block_data[0] =
            T5577_MOD_MANCHESTER | T5577_BITRATE_RF_32 | T5577_TRANS_BL_1_3;
...
void gallagher_write_send(void *proto)
{
    (void)proto;
...
const LFRFIDProtocolBase protocol_gallagher = {
    ...
    .write   = {
        .begin = (lfrfidProtocolWriteBegin)gallagher_write_begin,
        .send  = (lfrfidProtocolWriteSend)gallagher_write_send,
    },
