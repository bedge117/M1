/*
 * bit_lib.c — bit manipulation helpers.
 *
 * Ported from Flipper Zero firmware (lib/bit_lib/bit_lib.c, GPLv3), used by the
 * lfrfid protocol T5577 write encoders. furi_check/furi_crash are mapped to
 * assert(); the printf-based debug helpers are omitted for the embedded target.
 */
#include "bit_lib.h"
#include <assert.h>

#define furi_check(x)  assert(x)
#define furi_crash(m)  assert(0)
